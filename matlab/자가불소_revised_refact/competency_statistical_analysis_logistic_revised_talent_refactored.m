...
% 파일 경로 설정
% 리팩토링: 모든 설정을 load_config.m에서 로드
config = load_config();

% 로깅 시작 (리팩토링: Phase 2)
logger('INFO', '========================================');
logger('INFO', '역량 기반 인재유형 예측 분석 시작');
logger('INFO', '========================================');
logger('INFO', '분석 설정: extreme_group=%s, baseline=%s, n_bootstrap=%d', ...
    config.extreme_group_method, config.baseline_type, config.n_bootstrap);

% 기존 설정 (참고용):
% - 경로: hr_file, comp_file, output_dir
% - 분석: extreme_group_method ('all'), baseline_type ('weighted')
% - Bootstrap: n_bootstrap (5000), n_permutation (1000-5000)
% - 이상치 제거: outlier_removal.method ('none')
% - 성과 순위: performance_ranking, ordinal_levels, level_names
% - 그룹 정의: high_performers, low_performers, excluded_*
...
% 리팩토링: config에서 경로 가져오기
bootstrap_chart_filename = config.bootstrap_chart_file;

% Bootstrap 설정 (리팩토링: config에서 가져오기)
...
if ~use_cached
    ...
    % 리팩토링: config에서 값 가져오기
    if n_samples <= 100
        n_permutations = config.n_permutation_max;   % 작은 데이터: 더 많은 퍼뮤테이션
    elseif n_samples <= 500
        n_permutations = config.n_permutation_mid;   % 중간 데이터: 적당한 퍼뮤테이션
    else
        n_permutations = config.n_permutation_min;   % 큰 데이터: 최소 퍼뮤테이션
    end
...
%% ========================================================================
%                    성과 비교: 로지스틱 가중치 vs 단순 평균
% =========================================================================

fprintf('\n\n');
fprintf('════════════════════════════════════════════════════════════\n');
fprintf('        【성과 비교】 로지스틱 가중치 vs 단순 평균\n');
fprintf('════════════════════════════════════════════════════════════\n');

%% 1. 두 가지 점수 계산
% 단순 평균 점수 (모든 역량에 동일한 가중치)
simple_avg_score = mean(X_normalized, 2);

% 로지스틱 가중치 점수 (기존 계산된 weighted_scores 사용)
logistic_score = weighted_scores;

% 유효한 데이터만 사용
valid_idx = ~isnan(simple_avg_score) & ~isnan(logistic_score) & ~isnan(y_weight);
simple_avg_score = simple_avg_score(valid_idx);
logistic_score = logistic_score(valid_idx);
y_binary = y_weight(valid_idx);
n_samples = length(y_binary);

fprintf('\n분석 대상: %d명 (고성과자: %d명, 저성과자: %d명)\n', ...
    n_samples, sum(y_binary == 1), sum(y_binary == 0));

%% 2. Top-K 정확도 비교
top_k_percentages = [10, 20, 30, 50];

fprintf('\n【Top-K 정확도 비교】\n');
fprintf('──────────────────────────────────────────────────────────\n');
fprintf('상위 K%%     단순평균     로지스틱     개선율(%%p)   개선율(%%)\n');
fprintf('──────────────────────────────────────────────────────────\n');

top_k_simple_all = zeros(size(top_k_percentages));
top_k_logistic_all = zeros(size(top_k_percentages));

for idx = 1:length(top_k_percentages)
    k_pct = top_k_percentages(idx);
    k = round(n_samples * k_pct / 100);

    % 단순 평균 기준 상위 K명
    [~, idx_simple] = sort(simple_avg_score, 'descend');
    top_k_simple = idx_simple(1:k);
    accuracy_simple = sum(y_binary(top_k_simple) == 1) / k * 100;
    top_k_simple_all(idx) = accuracy_simple;

    % 로지스틱 기준 상위 K명
    [~, idx_logistic] = sort(logistic_score, 'descend');
    top_k_logistic = idx_logistic(1:k);
    accuracy_logistic = sum(y_binary(top_k_logistic) == 1) / k * 100;
    top_k_logistic_all(idx) = accuracy_logistic;

    improvement_pp = accuracy_logistic - accuracy_simple;
    improvement_pct = (accuracy_logistic - accuracy_simple) / accuracy_simple * 100;

    fprintf('상위 %2d%%    %6.2f%%      %6.2f%%      %+6.2f%%p    %+6.1f%%\n', ...
        k_pct, accuracy_simple, accuracy_logistic, improvement_pp, improvement_pct);
end

%% 3. Lift Score 계산 (랜덤 선발 대비)
base_rate = sum(y_binary == 1) / n_samples * 100;

fprintf('\n【Lift Score 분석】\n');
fprintf('──────────────────────────────────────────────────────────\n');
fprintf('전체 고성과자 비율: %.2f%%\n', base_rate);
fprintf('──────────────────────────────────────────────────────────\n');
fprintf('상위 K%%     단순평균      로지스틱      개선효과\n');
fprintf('           Lift         Lift\n');
fprintf('──────────────────────────────────────────────────────────\n');

for idx = 1:length(top_k_percentages)
    k_pct = top_k_percentages(idx);
    accuracy_simple = top_k_simple_all(idx);
    accuracy_logistic = top_k_logistic_all(idx);

    lift_simple = accuracy_simple / base_rate;
    lift_logistic = accuracy_logistic / base_rate;

    fprintf('상위 %2d%%     %.2fx         %.2fx         +%.2fx\n', ...
        k_pct, lift_simple, lift_logistic, lift_logistic - lift_simple);
end
...
% 현재 데이터 기반으로 시뮬레이션
prob_high = sum(y_binary == 1) / n_samples;

simple_avg_high_count = zeros(n_simulations, 1);
logistic_high_count = zeros(n_simulations, 1);
random_high_count = zeros(n_simulations, 1);

% 실제 데이터 기반 점수와 레이블의 상관계수 계산
corr_simple = corr(simple_avg_score, y_binary, 'Type', 'Spearman');
corr_logistic = corr(logistic_score, y_binary, 'Type', 'Spearman');
...
for sim = 1:n_simulations
    ...
    % 가상의 점수 생성 (실제 데이터 분포 기반)
    sim_simple = randn(n_applicants, 1);
    sim_logistic = randn(n_applicants, 1);

    % 고성과자에게 더 높은 점수 부여 (실제 상관관계 반영)
    high_idx_sim = find(sim_labels == 1);
    low_idx_sim = find(sim_labels == 0);

    % 단순 평균: 실제 상관계수 기반 변별력
    effect_simple = corr_simple * 1.5;  % 효과 크기
    sim_simple(high_idx_sim) = sim_simple(high_idx_sim) + effect_simple;

    % 로지스틱: 더 높은 변별력
    effect_logistic = corr_logistic * 1.5;
    sim_logistic(high_idx_sim) = sim_logistic(high_idx_sim) + effect_logistic;

    % 상위 n_select명 선발
    [~, idx_s] = sort(sim_simple, 'descend');
    [~, idx_l] = sort(sim_logistic, 'descend');

    simple_avg_high_count(sim) = sum(sim_labels(idx_s(1:n_select)) == 1);
    logistic_high_count(sim) = sum(sim_labels(idx_l(1:n_select)) == 1);
...
fprintf('시나리오: %d명 지원자 중 %d명 선발 (%d회 시뮬레이션)\n', ...
    n_applicants, n_select, n_simulations);
fprintf('전체 고성과자 비율: %.1f%%\n\n', prob_high * 100);
fprintf('선발 방법          평균 고성과자 수    표준편차\n');
fprintf('──────────────────────────────────────────────────────────\n');
fprintf('랜덤 선발          %.2f명              %.2f\n', ...
    mean(random_high_count), std(random_high_count));
fprintf('단순 평균          %.2f명              %.2f\n', ...
    mean(simple_avg_high_count), std(simple_avg_high_count));
fprintf('로지스틱 가중치    %.2f명              %.2f\n', ...
    mean(logistic_high_count), std(logistic_high_count));

fprintf('\n【개선 효과】\n');
fprintf('──────────────────────────────────────────────────────────\n');
improvement_vs_random = (mean(logistic_high_count) - mean(random_high_count)) / mean(random_high_count) * 100;
improvement_vs_simple = (mean(logistic_high_count) - mean(simple_avg_high_count)) / mean(simple_avg_high_count) * 100;

fprintf('로지스틱 vs 랜덤 선발:   평균 +%.2f명 (%.1f%% 개선)\n', ...
    mean(logistic_high_count) - mean(random_high_count), improvement_vs_random);
fprintf('로지스틱 vs 단순 평균:   평균 +%.2f명 (%.1f%% 개선)\n', ...
    mean(logistic_high_count) - mean(simple_avg_high_count), improvement_vs_simple);

%% 5. ROC/AUC 비교
[~, ~, ~, AUC_simple] = perfcurve(y_binary, simple_avg_score, 1);
[~, ~, ~, AUC_logistic] = perfcurve(y_binary, logistic_score, 1);

fprintf('\n【ROC/AUC 비교】\n');
fprintf('──────────────────────────────────────────────────────────\n');
fprintf('단순 평균 AUC:      %.4f\n', AUC_simple);
fprintf('로지스틱 AUC:       %.4f\n', AUC_logistic);
fprintf('AUC 개선:           +%.4f (%.1f%% 개선)\n', ...
    AUC_logistic - AUC_simple, (AUC_logistic - AUC_simple) / AUC_simple * 100);
...
% 서브플롯 1: Top-K 정확도 비교
subplot(1, 3, 1);
plot(top_k_percentages, top_k_simple_all, '-o', 'LineWidth', 2.5, 'MarkerSize', 10, ...
    'Color', [0.2, 0.6, 0.8], 'MarkerFaceColor', [0.2, 0.6, 0.8]);
hold on;
plot(top_k_percentages, top_k_logistic_all, '-s', 'LineWidth', 2.5, 'MarkerSize', 10, ...
    'Color', [0.8, 0.2, 0.2], 'MarkerFaceColor', [0.8, 0.2, 0.2]);
yline(base_rate, '--k', sprintf('전체 고성과자 비율 (%.1f%%)', base_rate), ...
    'LineWidth', 1.5, 'FontSize', 10);
xlabel('상위 K% 선발', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('고성과자 비율 (%)', 'FontSize', 12, 'FontWeight', 'bold');
title('Top-K 정확도 비교', 'FontSize', 14, 'FontWeight', 'bold');
legend('단순 평균', '로지스틱 가중치', 'Location', 'best', 'FontSize', 11);
grid on;
set(gca, 'FontSize', 11);

% 서브플롯 2: 개선 효과
subplot(1, 3, 2);
improvement_all = top_k_logistic_all - top_k_simple_all;
bar_handle = bar(top_k_percentages, improvement_all, 'FaceColor', [0.3, 0.7, 0.3]);
xlabel('상위 K% 선발', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('개선율 (%p)', 'FontSize', 12, 'FontWeight', 'bold');
title('로지스틱 가중치의 개선 효과', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
set(gca, 'FontSize', 11);
% 값 표시
for i = 1:length(top_k_percentages)
    text(top_k_percentages(i), improvement_all(i) + 0.5, ...
        sprintf('+%.1f%%p', improvement_all(i)), ...
        'HorizontalAlignment', 'center', 'FontSize', 10, 'FontWeight', 'bold');
end

% 서브플롯 3: 채용 시뮬레이션 결과
subplot(1, 3, 3);
methods = {'랜덤', '단순평균', '로지스틱'};
mean_counts = [mean(random_high_count), mean(simple_avg_high_count), mean(logistic_high_count)];
std_counts = [std(random_high_count), std(simple_avg_high_count), std(logistic_high_count)];

bar_handle = bar(mean_counts, 'FaceColor', [0.5, 0.5, 0.8]);
hold on;
errorbar(1:3, mean_counts, std_counts, 'k.', 'LineWidth', 1.5, 'CapSize', 10);
set(gca, 'XTickLabel', methods);
ylabel(sprintf('선발된 고성과자 수 (/%d명)', n_select), 'FontSize', 12, 'FontWeight', 'bold');
...
% 값 표시
for i = 1:3
    text(i, mean_counts(i) + std_counts(i) + 0.2, ...
        sprintf('%.2f±%.2f', mean_counts(i), std_counts(i)), ...
        'HorizontalAlignment', 'center', 'FontSize', 10, 'FontWeight', 'bold');
end

% 저장
sgtitle('로지스틱 가중치 vs 단순 평균 성과 비교', 'FontSize', 16, 'FontWeight', 'bold');
performance_comparison_file = get_managed_filepath(config.output_dir, 'performance_comparison.png', config);
backup_and_prepare_file(performance_comparison_file, config);
saveas(fig_comparison, performance_comparison_file);

fprintf('\n성과 비교 그래프 저장 완료: %s\n', performance_comparison_file);

% 결과 요약 저장
comparison_results = struct();
comparison_results.top_k_percentages = top_k_percentages;
comparison_results.simple_accuracy = top_k_simple_all;
comparison_results.logistic_accuracy = top_k_logistic_all;
comparison_results.improvement = improvement_all;
comparison_results.AUC_simple = AUC_simple;
comparison_results.AUC_logistic = AUC_logistic;
comparison_results.simulation = struct(...
    'n_applicants', n_applicants, ...
    'n_select', n_select, ...
    'n_simulations', n_simulations, ...
    'random_mean', mean(random_high_count), ...
    'simple_mean', mean(simple_avg_high_count), ...
    'logistic_mean', mean(logistic_high_count), ...
    'improvement_vs_random_pct', improvement_vs_random, ...
    'improvement_vs_simple_pct', improvement_vs_simple);

comparison_results_file = get_managed_filepath(config.output_dir, 'performance_comparison_results.mat', config);
backup_and_prepare_file(comparison_results_file, config);
save(comparison_results_file, 'comparison_results');

fprintf('성과 비교 결과 저장 완료: %s\n', comparison_results_file);
fprintf('════════════════════════════════════════════════════════════\n\n');
