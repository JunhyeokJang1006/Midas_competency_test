...
if ismember('Total_CSR_Score', csrCategories)
    fprintf('▶ Total CSR 점수 예측 모델 구축\n');

    % 종속변수
    dependentVar = finalCSRData.Total_CSR_Score;
    ...
    fprintf('  ⚠ Total_CSR_Score 카테고리를 찾을 수 없어 회귀분석 생략\n');
    regressionResults = [];
end
...
% 기존 파일 백업 (corr_item_vs_comp_score.m 방식)
existingFiles = dir(fullfile(outputDir, 'CSR_vs_competency_results*.xlsx'));
...
% MAT 파일도 백업
existingMatFiles = dir(fullfile(outputDir, 'CSR_workspace_*.mat'));
...
fprintf('----------------------------------------\n');
    ...
    % 시트명 매핑 테이블 생성 (corr_item_vs_comp_score.m 방식)
    sheetNameMapping = containers.Map();
    sheetNameMapping('OrganizationalSynergy') = 'Q40_조직시너지';
    sheetNameMapping('Pride') = 'Q41_자부심';
    sheetNameMapping('C1_Communication_Relationship') = 'Q42_소통관계';
    sheetNameMapping('C2_Communication_Purpose') = 'Q43_소통목적';
    sheetNameMapping('C3_Strategy_CustomerValue') = 'Q44_전략고객가치';
    sheetNameMapping('C4_Strategy_Performance') = 'Q45_전략성과';
    sheetNameMapping('C5_Reflection_Organizational') = 'Q46_성찰조직';
    sheetNameMapping('Communication_Score') = '소통영역_점수';
    sheetNameMapping('Strategy_Score') = '전략영역_점수';
    sheetNameMapping('Reflection_Score') = '성찰영역_점수';
    sheetNameMapping('Organizational_Score') = '조직영역_점수';
    sheetNameMapping('Total_CSR_Score') = 'CSR_총점';
    sheetNameMapping('Total_Leadership_Score') = '리더십_총점';
...
for i = 1:length(csrCategories)
    ...
    % 시트명이 비어있거나 숫자로 시작하는 경우 수정
    if isempty(sheetName) || isempty(regexp(sheetName, '^[A-Za-z]', 'once'))
        sheetName = ['Sheet_' sheetName];
    end
    ...
    try
        ...
        if isfield(correlationResults, csrCatName)
            ...
            % 빈 데이터 확인
            if isempty(corrData) || isempty(compCategories)
                fprintf('  ⚠ %s 데이터가 비어있음, 건너뜀\n', csrCatName);
                continue;
            end
...
if ~isempty(allSignificantResults)
    try
        ...
        try
            ...
            sigTable.Correlation = cellfun(@(x) double(x), allSignificantResults(:, 3));
        end
        
        try
            sigTable.P_Value = cell2mat(allSignificantResults(:, 4));
        catch
            sigTable.P_Value = cellfun(@(x) double(x), allSignificantResults(:, 4));
        end
        ...
        try
            ...
            sigTable.Sample_Size = cellfun(@(x) double(x), allSignificantResults(:, 6));
        end
...
if ~isempty(marginalResults)
    try
        ...
        try
            ...
            margTable.Correlation = cellfun(@(x) double(x), marginalResults(:, 3));
        end
        
        try
            margTable.P_Value = cell2mat(marginalResults(:, 4));
        catch
            margTable.P_Value = cellfun(@(x) double(x), marginalResults(:, 4));
        end
        ...
        try
            ...
            margTable.Sample_Size = cellfun(@(x) double(x), marginalResults(:, 6));
        end
...
if ~isempty(strongCorrelations)
    try
        ...
        try
            ...
            strongTable.Correlation = cellfun(@(x) double(x), strongCorrelations(:, 3));
        end
        
        try
            strongTable.P_Value = cell2mat(strongCorrelations(:, 4));
        catch
            strongTable.P_Value = cellfun(@(x) double(x), strongCorrelations(:, 4));
        end
        ...
        try
            ...
            strongTable.Sample_Size = cellfun(@(x) double(x), strongCorrelations(:, 6));
        end
...
%% 11-6. MAT 파일 저장 (corr_item_vs_comp_score.m 방식)
fprintf('\n▶ [11-6] MATLAB 작업공간 저장\n');
fprintf('----------------------------------------\n');

% 고정 파일명 사용 (백업 시스템으로 관리)
matFileName = fullfile(outputDir, 'CSR_vs_competency_workspace.mat');
...
try
    ...
    save(matFileName, ...
         ...
         'analysisType', 'extendedScores', 'extendedCategories', ...
         'validRows', 'commonIDs', 'csrIdx', 'compIdx', '-v7.3');
    ...
    % 원시 데이터도 저장 (분석 재현을 위해)
    if exist('csrRawData', 'var')
        save(matFileName, 'csrRawData', 'competencyTestData', 'upperCategoryData', '-append');
    end
