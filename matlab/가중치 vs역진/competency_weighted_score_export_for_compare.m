...
config = struct();
% talent 분석 결과 사용
config.weight_model_dir    = 'D:\project\HR데이터\결과\자가불소_revised_talent';  % talent 분석 가중치 디렉토리
config.new_output_dir      = 'D:\project\HR데이터\결과\자가불소_revised_talent';  % talent 분석 출력 디렉토리
config.reference_file      = 'D:\project\HR데이터\데이터\역량검사 요청 정보\23-25년 역량검사_개발자추가_filtered.xlsx';
...
config.output_filename     = sprintf('역량검사_가중치적용점수_talent_%s.xlsx', config.timestamp);
...
% integrated_analysis_results.mat 파일 우선 로드 시도
weight_file = fullfile(config.weight_model_dir, 'integrated_analysis_results.mat');
if ~exist(weight_file, 'file')
    % 백업: cost_sensitive_weights.mat 시도
...
loaded_data = load(weight_file);

fprintf('  ✓ 가중치 파일 로드: %s\n', weight_file);

% 통합 결과 파일인 경우
if isfield(loaded_data, 'integrated_results')
    fprintf('  → integrated_analysis_results.mat 형식 감지\n');
    integrated_results = loaded_data.integrated_results;

    % weight_comparison 테이블에서 Ensemble_Mean 사용
    if isfield(integrated_results, 'weight_comparison')
        weight_comp = integrated_results.weight_comparison;

        % Feature 컬럼에서 역량명 추출
        if ismember('Feature', weight_comp.Properties.VariableNames)
            feature_names = string(weight_comp.Feature);
        else
            feature_names = string(weight_comp{:, 1});
        end

        % Logistic 컬럼에서 가중치 추출
        if ismember('Logistic', weight_comp.Properties.VariableNames)
            final_weights = weight_comp.Logistic(:);
            fprintf('  → Logistic 가중치 사용\n');
        elseif ismember('logistic', weight_comp.Properties.VariableNames)
            final_weights = weight_comp.logistic(:);
            fprintf('  → Logistic 가중치 사용\n');
        else
            error('weight_comparison에 Logistic 컬럼이 없습니다.');
        end
    % 구형식: final_composite_weights 직접 사용
    elseif isfield(integrated_results, 'final_composite_weights') && ...
           isfield(integrated_results, 'feature_names')
        final_weights = integrated_results.final_composite_weights(:);
        feature_names = string(integrated_results.feature_names(:));
        fprintf('  → 통합 가중치(final_composite_weights) 사용\n');
    else
        error('integrated_results에 weight_comparison 또는 final_composite_weights가 없습니다.');
    end

% result_data 형식인 경우 (기존 방식)
elseif isfield(loaded_data, 'result_data')
    fprintf('  → cost_sensitive_weights.mat 형식 감지\n');
    result_data = loaded_data.result_data;
    ...
    feature_names = string(result_data.feature_names(:));
else
    error('가중치 파일 형식을 인식할 수 없습니다. integrated_results 또는 result_data가 필요합니다.');
end
...
if ~isempty(reliability_col_idx)
    fprintf('  ✓ 신뢰가능성 컬럼 발견: %s\n', comp_data.Properties.VariableNames{reliability_col_idx});
...
%% 5.5) 완전 케이스 필터링 옵션 (분석 일관성 유지) --------------------------
fprintf('\n【STEP 5.5】 완전 케이스 필터링 (선택 사항)\n');
fprintf('--------------------------------------------\n');

% 완전 케이스 필터링 옵션 (기본값: true, analysis 파일과 일관성 유지)
% analysis 파일과 동일하게 완전 케이스만 사용
config.use_complete_cases = true;  % false로 변경하면 결측값 허용

if config.use_complete_cases
    fprintf('  ⚙ 완전 케이스 필터링 활성화\n');

    % Step 1: 고품질 샘플 선택 (결측률 30% 미만)
    missing_rate_per_sample = sum(isnan(X), 2) / size(X, 2);
    ...
    % Step 2: 완전한 케이스만 사용
    complete_cases = quality_samples & ~any(isnan(X), 2);

    fprintf('  Step 2 - 완전한 케이스만 사용:\n');
    fprintf('    • 추가 제거: %d명\n', sum(quality_samples) - sum(complete_cases));
    fprintf('    • 최종 샘플: %d명\n', sum(complete_cases));

    % 필터링 적용
    X = X(complete_cases, :);
    common_ids = common_ids(complete_cases);
    hr_matched = hr_matched(complete_cases, :);
    comp_matched = comp_matched(complete_cases, :);
    n = size(X, 1);

    fprintf('  ✓ 완전 케이스 필터링 완료: %d명\n', n);
else
    fprintf('  ℹ 완전 케이스 필터링 비활성화 (결측값 허용, 행별 가중치 재정규화)\n');
    fprintf('    • 전체 샘플: %d명\n', n);
end
...
% 최신 소화성 분석 결과 파일 찾기
sohwa_result_dir = 'D:\project\HR데이터\결과\소화성분석_talent';  % talent 분석 결과 디렉토리
sohwa_model_file = '';
...
if exist(sohwa_result_dir, 'dir')
    ...
    if ~isempty(files)
        ...
        fprintf('  ⚠ 소화성 모델 파일이 없습니다: %s\n', sohwa_result_dir);
    end
else
    fprintf('  ⚠ 소화성 결과 디렉토리가 없습니다: %s\n', sohwa_result_dir);
end
...
if exist(sohwa_model_file, 'file')
    try
        ...
        if isfield(sohwa_results, 'logit_model') && ...
           isfield(sohwa_results, 'zscore_params')
            ...
            % 조정된 점수 계산
            adjusted_score = score_100 .* (1 - sohwa_probs * adjustment_strength);
            adjusted_score(isnan(adjusted_score)) = score_100(isnan(adjusted_score));  % NaN인 경우 원래 점수 유지
            adjusted_score = round(adjusted_score);

            result.('소화성조정점수') = adjusted_score;
            result.('점수변화') = score_100 - adjusted_score;

            fprintf('    • 조정 강도: %.2f\n', adjustment_strength);
            fprintf('    • 조정 전 평균: %.2f ± %.2f\n', mean(score_100, 'omitnan'), std(score_100, 'omitnan'));
            fprintf('    • 조정 후 평균: %.2f ± %.2f\n', mean(adjusted_score, 'omitnan'), std(adjusted_score, 'omitnan'));
            fprintf('    • 평균 감소: %.2f점\n', mean(score_100 - adjusted_score, 'omitnan'));

        else
            fprintf('  ⚠ 소화성 모델 또는 파라미터 없음\n');
            result.('소화성확률_퍼센트') = nan(n, 1);
            result.('소화성조정점수') = score_100;
            result.('점수변화') = zeros(n, 1);
        end
    catch ME
    ...
    fprintf('  ⚠ 소화성 모델 파일 없음\n');
    if ~isempty(sohwa_model_file)
        fprintf('    경로: %s\n', sohwa_model_file);
    end
    result.('소화성확률_퍼센트') = nan(n, 1);
    result.('소화성조정점수') = score_100;
...
% 기존 파일 백업 (역량검사_가중치적용점수_talent로 시작하는 모든 파일)
existing_files = dir(fullfile(config.new_output_dir, '역량검사_가중치적용점수_talent*.xlsx'));
...
meta.('항목') = { ...
    ...
    '전부결측_NaN행_수'; ...
    '소화성모델_적용여부'; '소화성확률_계산샘플수'; ...
    '조정강도_파라미터'; '조정후_평균점수'; '평균_점수감소' ...
    };

% 소화성 통계 계산
sohwa_applied = ~all(isnan(result.('소화성확률_퍼센트')));
sohwa_count = sum(~isnan(result.('소화성확률_퍼센트')));
if sohwa_applied && ismember('소화성조정점수', result.Properties.VariableNames)
    adj_mean = mean(result.('소화성조정점수'), 'omitnan');
    avg_decrease = mean(result.('점수변화'), 'omitnan');
    sohwa_status = '적용됨';
    adjustment_str = 0.5;
else
    adj_mean = nan;
...
meta.('값') = { ...
    ...
    'competency_weighted_score_export_revised_talent.m'; ...
    config.comp_sheet; ...
    ...
    sohwa_status; ...
    sprintf('%d', sohwa_count); ...
    sprintf('%.2f', adjustment_str); ...
    sprintf('%.2f', adj_mean); ...
    sprintf('%.2f', avg_decrease) ...
    };
