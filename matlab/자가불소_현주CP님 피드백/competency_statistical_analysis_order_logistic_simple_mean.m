...
if exist('distribution_stats', 'var') && istable(distribution_stats)
    ...
    fprintf('  디버그: 사용 가능한 필드 - %s\n', strjoin(field_names, ', '));
else
    fprintf('  ⚠ distribution_stats 테이블이 없습니다.\n');
end
...
for i = 1:n_types
    ...
    if ~islogical(type_mask)
        fprintf('  ⚠ type_mask가 logical이 아님: %s\n', class(type_mask));
        type_mask = logical(type_mask);
    end

    % matched_comp 데이터 안전하게 접근
...
% profile_stats 테이블 존재 및 필드 확인
if ~exist('profile_stats', 'var') || ~istable(profile_stats)
    fprintf('  ⚠ profile_stats 테이블이 없습니다. 단순 평균으로 대체합니다.\n');
    sample_counts = ones(n_types, 1);  % 동일 가중치
elseif ~ismember('Count', profile_stats.Properties.VariableNames)
    fprintf('  ⚠ profile_stats.Count 필드가 없습니다. 동일 가중치를 사용합니다.\n');
...
if ~exist('n_types', 'var') || isempty(n_types)
    if exist('unique_matched_types', 'var')
        ...
        fprintf('  ⚠ n_types 변수를 찾을 수 없습니다. 기본값 5를 사용합니다.\n');
        n_types = 5;
    end
end
...
if ~exist('type_profiles', 'var') || isempty(type_profiles)
    fprintf('  ⚠ type_profiles 데이터가 없습니다. 개별 레이더 차트 생성을 건너뜁니다.\n');
    n_types = 0;
end

if ~exist('unique_matched_types', 'var') || isempty(unique_matched_types)
    fprintf('  ⚠ unique_matched_types가 없습니다. 개별 레이더 차트 생성을 건너뜁니다.\n');
    n_types = 0;
end
    ...
    if use_parallel
        ...
        parfor perm = 1:n_permutations
            try
                ...
                % best_method에 따라 다른 방법으로 퍼뮤테이션 성능 측정
                perm_n = length(shuffled_y);

                % best_method별 가중치 계산 및 예측
                switch best_method_temp
                    case 'Correlation'
                        % 상관계수 기반 가중치 계산 (shuffled label 사용)
                    ...
                    case 'Bootstrap'
                        % 간단한 부트스트랩 평균 (퍼뮤테이션에서는 축약 버전)
                        perm_weights = zeros(size(X_normalized_temp, 2), 1);
                        n_boot_simple = 100; % 퍼뮤테이션 내에서는 적은 수
        ...
        for perm = 1:n_permutations
            try
                ...
                % best_method에 따라 다른 방법으로 퍼뮤테이션 성능 측정 (순차 처리)
                perm_n = length(shuffled_y);
...
fprintf('   - 작은 데이터에서 안정적인 성능 추정\n');
    ...
    % 필수 변수가 없는 경우 처리
    fprintf('  ⚠ 필수 변수 누락으로 STEP 22.5 건너뛰기\n');
    if ~exist('X_normalized', 'var')
        fprintf('    - X_normalized 변수 누락\n');
    end
    if ~exist('y_weight', 'var')
        fprintf('    - y_weight 변수 누락\n');
    end
    if ~exist('prediction_results', 'var')
        fprintf('    - prediction_results 변수 누락\n');
    end
    if ~exist('best_method', 'var')
        fprintf('    - best_method 변수 누락\n');
    end

    % 빈 구조체 생성 (후속 단계에서 사용 가능하도록)
    permutation_results = struct();
    permutation_results.f1 = struct();
    permutation_results.auc = struct();
    permutation_results.meta = struct();

    fprintf('  ✓ STEP 22.5 건너뛰기 완료\n');
end

%% STEP 22.6: 모든 가중치 방법 퍼뮤테이션 테스트 비교
fprintf('\n\n【STEP 22.6】 모든 가중치 방법 퍼뮤테이션 테스트 비교\n');
fprintf('════════════════════════════════════════════════════════════\n');
...
n_permutations=5000
% 예상 시간 계산 및 사용자 알림 (실제 처리할 방법 수 기준)
...
for method_idx = 1:n_available_methods
    ...
    if ~use_cached
        ...
        % 관찰된 성능 가져오기 (이미 필터링되었으므로 데이터 존재 보장)
        ...
        if use_parallel
            ...
            for perm = 1:n_permutations
                % 동일한 퍼뮤테이션 로직 (위와 동일, 진행률 표시 추가)
                try
                    shuffled_y = y_weight(randperm(n_samples));
                    perm_n = length(shuffled_y);

                    % 방법별 처리 (위와 동일한 switch 구문)
...
for method_idx = 1:n_available_methods
    ...
    % Figure 생성 (겹치지 않게 위치 조정)
    fig = figure('Name', sprintf('LOOCV 퍼뮤테이션 - %s', method), ...
                 'Position', [100+method_idx*50, 100+method_idx*30, 1800, 1000]);

    % 6개 subplot 생성 (기존과 동일한 구조)

    % AUC 히스토그램
    subplot(2, 3, 1);
    histogram(results.auc.null_distribution, 40, 'Normalization', 'probability', 'FaceAlpha', 0.7);
    hold on;
    xline(results.auc.observed, 'r-', 'LineWidth', 3, 'DisplayName', sprintf('관찰 AUC (%.3f)', results.auc.observed));
    xline(results.auc.mean_null, 'k--', 'LineWidth', 2, 'DisplayName', sprintf('평균 (%.3f)', results.auc.mean_null));
    title(sprintf('%s: AUC 귀무분포 (p=%.4f)', method, results.auc.p_value), 'FontWeight', 'bold');
    xlabel('AUC'); ylabel('확률밀도');
    legend('Location', 'best');
    grid on;
    ...
    hold on;
    xline(results.f1.observed, 'r-', 'LineWidth', 3, 'DisplayName', sprintf('관찰 F1 (%.3f)', results.f1.observed));
    xline(results.f1.mean_null, 'k--', 'LineWidth', 2, 'DisplayName', sprintf('평균 (%.3f)', results.f1.mean_null));
    title(sprintf('%s: F1 귀무분포 (p=%.4f)', method, results.f1.p_value), 'FontWeight', 'bold');
    xlabel('F1 Score'); ylabel('확률밀도');
    legend('Location', 'best');
    grid on;
    ...
    title(sprintf('%s: AUC 정규성 검정', method), 'FontWeight', 'bold');

    % F1 Q-Q Plot
    subplot(2, 3, 4);
    qqplot(results.f1.null_distribution);
    title(sprintf('%s: F1 정규성 검정', method), 'FontWeight', 'bold');
    ...
    title(sprintf('%s: AUC vs F1 관계', method), 'FontWeight', 'bold');
    grid on;
    ...
    summary_text = {
        sprintf('【%s 요약】', method);
        '';
        sprintf('관찰 AUC: %.4f', results.auc.observed);
        sprintf('귀무평균: %.4f ± %.4f', results.auc.mean_null, results.auc.std_null);
        sprintf('p-value: %.4f', results.auc.p_value);
        sprintf('백분위: %.1f%%', results.auc.percentile_rank);
        '';
        sprintf('관찰 F1: %.4f', results.f1.observed);
        sprintf('귀무평균: %.4f ± %.4f', results.f1.mean_null, results.f1.std_null);
        sprintf('p-value: %.4f', results.f1.p_value);
        sprintf('백분위: %.1f%%', results.f1.percentile_rank);
        '';
        sprintf('퍼뮤테이션: %d회', results.meta.n_permutations);
        sprintf('소요시간: %.1f초', results.meta.elapsed_time);
    };
...
% 통합 비교 시각화 생성
fprintf('\n【통합 비교 시각화 생성】\n');
comp_fig = figure('Name', '5가지 방법 퍼뮤테이션 비교', 'Position', [50, 50, 1800, 1200]);

% AUC p-value 비교
subplot(2, 3, 1);
bar(comparison_table.AUC_p_value);
set(gca, 'XTickLabel', all_methods, 'XTickLabelRotation', 45);
title('AUC p-value 비교', 'FontWeight', 'bold');
ylabel('p-value');
yline(0.05, 'r--', 'LineWidth', 2, 'DisplayName', 'α=0.05');
grid on;
...
set(gca, 'XTickLabel', all_methods, 'XTickLabelRotation', 45);
title('F1 p-value 비교', 'FontWeight', 'bold');
ylabel('p-value');
yline(0.05, 'r--', 'LineWidth', 2, 'DisplayName', 'α=0.05');
grid on;
...
set(gca, 'XTickLabel', all_methods, 'XTickLabelRotation', 45);
title('효과크기 (Z-score) 비교', 'FontWeight', 'bold');
ylabel('Z-score');
...
xlabel('관찰 AUC'); ylabel('관찰 F1');
title('관찰값 분포', 'FontWeight', 'bold');
grid on;
...
sgtitle('5가지 방법 퍼뮤테이션 테스트 종합 비교', 'FontSize', 16, 'FontWeight', 'bold');
...
try
    ...
    fprintf('  ✓ 통합 비교 시각화 저장: %s\n', comp_fig_filename);
catch
    fprintf('  ⚠ 통합 비교 시각화 저장 실패\n');
end
...
fprintf('  - 개별 시각화 %d개 생성\n', n_available_methods);
fprintf('  - 통합 비교 시각화 1개 생성\n');
fprintf('  - 캐시 파일 %d개 저장\n', n_available_methods);

end % 사용자 확인 if문 끝

end % n_available_methods == 0 조건문 끝
...
try
    ...
    %% 9.5. 퍼뮤테이션비교 시트 (STEP 22.6 결과)
    if exist('all_permutation_results', 'var') && exist('comparison_table', 'var')
        fprintf('  • 퍼뮤테이션비교 시트 생성 중...\n');
        writetable(comparison_table, excel_filepath, 'Sheet', '퍼뮤테이션비교');
        ...
        for method_idx = 1:length(methods_to_save)
            ...
            fprintf('  • %s_퍼뮤테이션 시트 생성 중...\n', method_name);

            % 상세 결과 테이블 생성
            detail_table = table();
            detail_table.('항목') = {
                '방법명'; '관찰AUC'; 'AUC귀무평균'; 'AUC귀무표준편차'; 'AUC_p값'; 'AUC_Z점수';
                '관찰F1'; 'F1귀무평균'; 'F1귀무표준편차'; 'F1_p값'; 'F1_Z점수';
                '퍼뮤테이션횟수'; '소요시간초'; '실패횟수'
            };
            ...
            sheet_name = sprintf('%s_퍼뮤테이션', method_name);
    ...
    % Cut-off 효과 분석 시트 (신규)
    if exist('distribution_stats', 'var') && height(distribution_stats) > 0
        fprintf('  • Cut-off분석 시트 생성 중...\n');

        % Cut-off 효과가 있는 역량들만 필터링
        ...
        if any(cutoff_affected)
            ...
            for idx = 1:sum(cutoff_affected)
                ...
                if contains(types, 'Floor_Effect')
                    cutoff_analysis_table.Recommendation{idx} = '점수 하한선 조정 검토 필요';
                elseif contains(types, 'Ceiling_Effect')
                    cutoff_analysis_table.Recommendation{idx} = '점수 상한선 조정 또는 난이도 증가 필요';
                elseif contains(types, 'Lower_Truncation')
                    cutoff_analysis_table.Recommendation{idx} = '선발 기준 완화 검토 필요';
                elseif contains(types, 'Upper_Truncation')
                    cutoff_analysis_table.Recommendation{idx} = '상위 인재 선발 확대 검토 필요';
                else
                    cutoff_analysis_table.Recommendation{idx} = '분포 정상화를 위한 추가 검토 필요';
                end
            end

            writetable(cutoff_analysis_table, excel_filepath, 'Sheet', 'Cut-off분석');
        else
            % Cut-off 효과가 없는 경우 요약 정보만
            summary_table = table();
            summary_table.('분석결과') = {'Cut-off 효과 없음'};
            summary_table.('전체역량수') = {size(distribution_stats, 1)};
            summary_table.('정상분포역량수') = {sum(~distribution_stats.Has_Cutoff)};
            summary_table.('비고') = {'모든 역량이 정상 분포를 보임'};
            writetable(summary_table, excel_filepath, 'Sheet', 'Cut-off분석');
        end

        % Cut-off 요약 통계
        if exist('cutoff_summary', 'var')
            summary_stats_table = table();
            summary_stats_table.('항목') = {'전체_역량수'; 'Cut-off_효과_역량수'; 'Floor_효과_수'; 'Ceiling_효과_수'; ...
                                      '하위_절단_수'; '상위_절단_수'; 'Cut-off_발생률_퍼센트'};
            summary_stats_table.('값') = [cutoff_summary.total_competencies; cutoff_summary.with_cutoff;
            ...
            writetable(summary_stats_table, excel_filepath, 'Sheet', 'Cut-off요약통계');
            fprintf('  • Cut-off요약통계 시트 생성 중...\n');
        end
    end
    ...
    if exist('all_permutation_results', 'var') && exist('comparison_table', 'var')
        sheet_count = sheet_count + 1; % 퍼뮤테이션비교 시트
        sheet_count = sheet_count + length(fieldnames(all_permutation_results)); % 각 방법별 시트
    end
...
function value = safe_get_field(struct_var, field_names, default_value)
    % 여러 가능한 필드명 중 첫 번째로 찾은 값 반환
    % struct_var: 검색할 구조체
    % field_names: 가능한 필드명들 (문자열 또는 셀 배열)
    % default_value: 필드를 찾지 못했을 때 반환할 기본값
...
function field_exists = safe_check_fields(struct_var, required_fields)
    % 구조체에 필수 필드들이 모두 존재하는지 확인
    % struct_var: 검색할 구조체
    % required_fields: 필수 필드명들 (셀 배열)
    % field_exists: 모든 필드가 존재하면 true, 아니면 false
...
function struct_out = safe_init_struct(field_names, default_values)
    % 안전하게 구조체를 초기화하는 함수
    % field_names: 필드명들 (셀 배열)
    % default_values: 각 필드의 기본값들 (셀 배열)
