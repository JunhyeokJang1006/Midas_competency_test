...
%% 초기화 및 설정
% 기존 변수 완전 초기화
clear allData consolidatedScores periods;

% 기본 설정 상수
MAX_PERIODS = 5;  % 최대 period 수 정의

% 결과 저장 디렉토리 통일 설정
resultDir = 'D:\project\HR데이터\matlab\문항기반_revised';
...
fprintf(' * 메탃데이터 파일: question_scale_metadata_with_23_rebuilt.xlsx\n');
fprintf('========================================\n\n');
...
%% 데이터 구조 검증 및 초기화
function validateDataStructure(allData, periods)
    fprintf('========================================\n');
    fprintf('데이터 구조 검증\n');
    fprintf('========================================\n');
    ...
    if ~isempty(fieldnames(allData))
        ...
        fprintf('현재 allData 필드:\n');
        for i = 1:length(allDataFields)
            fprintf('  - %s\n', allDataFields{i});
        end

        % period 번호 일관성 확인
        expectedPeriods = 1:length(allDataFields);
        actualPeriods = [];
        for i = 1:length(allDataFields)
            periodNum = extractPeriodNumber(allDataFields{i});
            actualPeriods(end+1) = periodNum;
        end

        if ~isequal(sort(actualPeriods), expectedPeriods)
            warning('Period 번호가 연속적이지 않습니다!');
            fprintf('예상: %s\n', num2str(expectedPeriods));
            fprintf('실제: %s\n', num2str(sort(actualPeriods)));
        end
    end

    % periods 배열과 allData 일치 확인
    if length(periods) ~= length(fieldnames(allData))
        warning('periods 배열과 allData 필드 개수가 일치하지 않습니다!');
        fprintf('periods: %d개, allData fields: %d개\n', ...
                length(periods), length(fieldnames(allData)));
    end
end

function allData = fixPeriodStructure(allData, expectedPeriods)
    % Period 구조 수정 함수
    tempData = struct();
    fields = fieldnames(allData);

    % 올바른 period만 추출
    for i = 1:min(expectedPeriods, length(fields))
        if isfield(allData, sprintf('period%d', i))
            tempData.(sprintf('period%d', i)) = allData.(sprintf('period%d', i));
        end
    end
...
if ~isempty(matFiles)
    ...
    try
        ...
        if isfield(loadedData, 'allData')
            ...
            % 데이터 구조 검증
            validateDataStructure(allData, periods);

            % Period 개수가 비정상적으로 많은 경우 수정
            if length(fieldnames(allData)) > 5
                fprintf('⚠️ Period 개수 조정 중...\n');
                allData = fixPeriodStructure(allData, 5);
            end
...
if ~isempty(fieldnames(allData))
    allDataFields = fieldnames(allData);
    fprintf('현재 allData 구조: %d개 period\n', length(allDataFields));

    % Period 개수 검증 및 제한
    if length(allDataFields) > MAX_PERIODS
        warning('Period 개수가 %d개로 예상보다 많습니다. %d개로 제한합니다.', length(allDataFields), MAX_PERIODS);

        % 초과 period 제거
        for p = (MAX_PERIODS + 1):length(allDataFields)
            fieldName = sprintf('period%d', p);
            if isfield(allData, fieldName)
                allData = rmfield(allData, fieldName);
                fprintf('  제거됨: %s\n', fieldName);
            end
        end
    end

    % period1이 이미 23년 상반기인지 확인
    if isfield(allData, 'period1') && isfield(allData.period1, 'selfData')
        period1Data = allData.period1.selfData;
        if height(period1Data) > 0
            % period1이 30명 내외라면 23년 상반기일 가능성이 높음
            % (기존 4개 period는 모두 79-94명 범위)
            if height(period1Data) < 50
                fprintf('  ⚠️ period1이 이미 존재하며 23년 상반기로 추정됩니다 (%d명).\n', height(period1Data));
                fprintf('  ⚠️ 23년 상반기 데이터 추가를 건너띁니다.\n');
                skip23_1st = true;
            end
        end
    end
...
if ~skip23_1st && exist(fileName_23_1st, 'file')
    try
        ...
        % Period 재배열 (안전한 방식)
        fprintf('  ▶ Period 재배열 수행 중...\n');

        % 기존 field들을 정렬하여 순서 보장
        existingFields = fieldnames(temp_allData);
        sortedFields = sort(existingFields);

        fprintf('    기존 period: %d개\n', length(sortedFields));

        % 기존 period를 순서대로 재배치 (period2부터)
        for i = 1:length(sortedFields)
            newPeriodNum = i + 1;
            if newPeriodNum <= MAX_PERIODS  % 최대 5개 period로 제한
                allData.(sprintf('period%d', newPeriodNum)) = temp_allData.(sortedFields{i});
                fprintf('    %s -> period%d\n', sortedFields{i}, newPeriodNum);
            else
                fprintf('    %s 제외 (최대 period 초과)\n', sortedFields{i});
            end
        end
        ...
        % 데이터 무결성 검증
        actualPeriodCount = length(fieldnames(allData));
        if actualPeriodCount ~= length(periods)
            warning('예상 period 개수(%d)와 실제 period 개수(%d)가 일치하지 않습니다', length(periods), actualPeriodCount);
        end

        % period 번호 연속성 검증
        allDataFields = fieldnames(allData);
        expectedFields = cell(1, actualPeriodCount);
        for i = 1:actualPeriodCount
            expectedFields{i} = sprintf('period%d', i);
        end

        if ~isequal(sort(allDataFields), sort(expectedFields))
            warning('period 번호가 연속적이지 않습니다!');
            fprintf('    예상: %s\n', strjoin(expectedFields, ', '));
            fprintf('    실제: %s\n', strjoin(sort(allDataFields), ', '));
        end

        fprintf('  ✓ 23년 상반기 데이터 추가 완료: %d명\n', height(allData.period1.selfData));
        fprintf('  ✓ 전체 Period: %d개 (예상: %d개)\n', actualPeriodCount, length(periods));
        
    catch ME
        fprintf('  ✗ 23년 상반기 데이터 로드 실패: %s\n', ME.message);
        fprintf('  → 기존 데이터를 복구하고 계속 진행합니다\n');

        % 실패 시 기존 데이터 복구
        if exist('temp_allData', 'var')
            allData = temp_allData;
        end
    end
elseif ~skip23_1st
    fprintf('  • 23년 상반기 파일 없음 - 기존 데이터로 진행\n');
else
    fprintf('  ✓ 23년 상반기 데이터 추가를 건너띀어 기존 데이터로 진행\n');
end
...
% performanceQuestions 동적 조정
actualPeriodCount = length(fieldnames(allData));
performanceFields = fieldnames(performanceQuestions);

fprintf('  ▶ performanceQuestions 동적 조정 중...\n');
fprintf('    실제 period 수: %d, 기본 performanceQuestions: %d\n', actualPeriodCount, length(performanceFields));

% 실제 period 수에 맞춰 조정
if actualPeriodCount < length(performanceFields)
    % 초과 필드 제거
    fprintf('    초과 performanceQuestions 제거 중...\n');
    for i = (actualPeriodCount + 1):length(performanceFields)
        fieldName = sprintf('period%d', i);
        if isfield(performanceQuestions, fieldName)
            performanceQuestions = rmfield(performanceQuestions, fieldName);
            fprintf('      제거: %s\n', fieldName);
        end
    end
elseif actualPeriodCount > length(performanceFields)
    % 부족한 필드 추가 (마지막 period와 동일한 문항 사용)
    fprintf('    부족한 performanceQuestions 추가 중...\n');
    lastField = performanceFields{end};
    for i = (length(performanceFields) + 1):actualPeriodCount
        fieldName = sprintf('period%d', i);
        performanceQuestions.(fieldName) = performanceQuestions.(lastField);
        fprintf('      추가: %s (기본값: %s)\n', fieldName, lastField);
    end
else
    fprintf('    performanceQuestions와 period 수가 일치합니다\n');
end

fprintf('    ✓ 최종 performanceQuestions: %d개\n', length(fieldnames(performanceQuestions)));

%% 최종 데이터 무결성 검증
fprintf('\n[최종 검증] 데이터 무결성 찴크\n');
fprintf('----------------------------------------\n');

% allData와 periods 일치성 찴크
finalPeriodCount = length(fieldnames(allData));
finalPeriodsCount = length(periods);

fprintf('최종 period 개수: %d개\n', finalPeriodCount);
fprintf('최종 periods 배열: %d개\n', finalPeriodsCount);
fprintf('최종 performanceQuestions: %d개\n', length(fieldnames(performanceQuestions)));

if finalPeriodCount == finalPeriodsCount && finalPeriodCount == length(fieldnames(performanceQuestions))
    fprintf('✓ 모든 구조가 일치합니다\n');
else
    warning('구조 불일치 발견!');
end
...
for p = 1:length(periods)
    ...
    % 안전한 period 접근
    if p > length(fieldnames(allData))
        fprintf('  [경고] Period %d는 allData에 없습니다\n', p);
        continue;
    end

    periodField = sprintf('period%d', p);
    if ~isfield(allData, periodField)
        fprintf('  [경고] %s 필드를 찾을 수 없습니다\n', periodField);
        continue;
    end
...
for i = 1:length(periodFields)
    ...
    periodNum = extractPeriodNumber(fieldName);
...
if exist('upperCategoryResults', 'var') && ~isempty(upperCategoryResults)
    ...
    for periodIdx = 1:length(periodFields)
        ...
        % 100점 표준화 수행
        % periodField에서 period 번호 추출 (예: 'period1' -> 1)
        periodNum = str2double(regexp(periodField, '\d+', 'match', 'once'));
        if periodNum <= length(periods)
            periodName = periods{periodNum};
        else
            fprintf('  ⚠ Period %d는 periods 배열 범위를 초과합니다\n', periodNum);
            periodName = sprintf('Period_%d', periodNum);
        end
        fprintf('  - %s (%s) 표준화 진행\n', periodField, periodName);
                    ...
                    if count <= 10 && ismember('QuestionID', questionInfo.Properties.VariableNames) && ...
                       ...
                       ismember('Max_Scale', questionInfo.Properties.VariableNames)
                        ...
                        for ex = 1:min(5, height(exampleRows))
                            ...
                            % 숫자 타입으로 변환
                            if isnumeric(minS) && isnumeric(maxS) && ~isnan(minS) && ~isnan(maxS)
...
%% 유틸리티 함수들
function periodNum = extractPeriodNumber(fieldName)
    % period 필드명에서 숫자 추출하는 통일된 함수
    periodNumStr = regexp(fieldName, '\d+', 'match', 'once');
    if isempty(periodNumStr)
        periodNum = NaN;
    else
        periodNum = str2double(periodNumStr);
    end
end