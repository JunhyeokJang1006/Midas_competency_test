...
%% 8. 성과 관련 문항 종합점수 분석 (신뢰도 검증 포함)
fprintf('\n[5단계] 성과 관련 문항 종합점수 분석 (신뢰도 검증 포함)\n');
fprintf('========================================\n');
...
for p = 1:length(periods)
    ...
    if ~isempty(reliabilityResult.optimizedItems)
        ...
        fprintf('  ✅ 신뢰도 검증된 문항으로 성과점수 재계산 완료\n');
    else
        optimizedPerformanceScores = originalPerformanceScores;
        fprintf('  ⚠️  신뢰도 검증 실패 - 원본 점수 사용\n');
    end
    ...
    if size(cleanOriginalCorrelationData, 1) < 3 || size(cleanOptimizedCorrelationData, 1) < 3
        fprintf('  [경고] 역량검사-성과점수 상관분석을 위한 데이터가 부족합니다\n');
        continue;
    end
...
%% 9. 신뢰도 분석 및 성과점수 분석 결과 저장
if ~isempty(fieldnames(performanceResults))
    fprintf('\n[6단계] 신뢰도 분석 및 성과점수 상관분석 결과 저장\n');
    fprintf('========================================\n');

    % 1) 성과점수 분석 요약 테이블 생성 (기존)
    ...
    try
        ...
        % 새로운 시트들 저장
        writetable(reliabilityTable, outputFileName, 'Sheet', '성과문항_신뢰도분석');
        fprintf('✓ 성과문항 신뢰도분석 결과 저장 완료\n');

        writetable(reliabilityBasedPerfTable, outputFileName, 'Sheet', '신뢰도기반_성과점수');
        fprintf('✓ 신뢰도 기반 성과점수 결과 저장 완료\n');

        writetable(methodComparisonTable, outputFileName, 'Sheet', '방법론_비교');
        fprintf('✓ 방법론 비교 결과 저장 완료\n');

    catch ME
        fprintf('✗ Excel 파일 저장 실패: %s\n', ME.message);
    end
...
function perfSummaryTable = createPerformanceSummaryTable(performanceResults, periods)
    ...
    for i = 1:length(perfFields)
        ...
        if isfield(result, 'competencyMean')
            ...
            % 새로운 구조에서는 competencyTestScores에서 직접 계산
        ...
        if isfield(result, 'performanceMean')
            ...
            % 새로운 구조에서는 performanceScores에서 직접 계산
...
function alpha = calculateCronbachAlpha(data)
    % Cronbach's Alpha 계산
    % 입력: data - N x k 매트릭스 (N: 응답자 수, k: 문항 수)
    % 출력: alpha - Cronbach's Alpha 값
...
function itemTotalCorrs = calculateItemTotalCorrelations(data)
    % 각 문항과 나머지 문항들의 합계 간 상관계수 계산
    % 입력: data - N x k 매트릭스
    % 출력: itemTotalCorrs - k x 1 벡터 (각 문항의 item-total correlation)
...
%% 신뢰도 최적화 함수 (문항 제거를 통한 개선)
function [optimizedItems, optimizedAlpha, removedItems, alphaHistory] = optimizeReliability(data, itemNames, targetAlpha)
    % 낮은 item-total correlation을 가진 문항을 순차적으로 제거하여 신뢰도 개선
    % 입력:
    %   data - N x k 매트릭스
    %   itemNames - k x 1 cell array (문항 이름들)
    %   targetAlpha - 목표 Cronbach's Alpha 값 (기본값: 0.7)
    % 출력:
    %   optimizedItems - 최종 선택된 문항들의 인덱스
    %   optimizedAlpha - 최종 Cronbach's Alpha
    %   removedItems - 제거된 문항들의 정보
    %   alphaHistory - 각 단계별 Alpha 변화
    ...
    maxIterations = size(data, 2) - 2; % 최소 2개 문항은 남겨둠
...
function reliabilityResults = analyzePerformanceReliability(performanceData, itemNames, periodName)
    % 성과 문항들의 신뢰도 분석을 수행하는 함수

    fprintf('\n▶ %s 성과 문항 신뢰도 분석\n', periodName);
    fprintf('  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

    % 1. 원본 Cronbach's Alpha 계산
    originalAlpha = calculateCronbachAlpha(performanceData);
    fprintf('  📊 원본 Cronbach''s Alpha: %.3f', originalAlpha);
    ...
    if originalAlpha < 0.7
        ...
        for step = 1:length(removedItems)
            fprintf('    %d단계: ''%s'' 제거 (item-total r=%.3f) → α: %.3f → %.3f\n', ...
        ...
        fprintf('  📈 개선된 Cronbach''s Alpha: %.3f', optimizedAlpha);

        if optimizedAlpha >= 0.7
            fprintf(' (목표 달성 ✓)\n');
            finalReliabilityLevel = '수용가능';
        else
            fprintf(' (목표 미달성 ✗)\n');
            finalReliabilityLevel = '불량';
        end

        improvementAmount = optimizedAlpha - originalAlpha;
        fprintf('  📊 신뢰도 개선 정도: +%.3f\n', improvementAmount);

    else
        fprintf('\n  ✅ 원본 신뢰도가 이미 수용가능한 수준입니다.\n');
...
function methodComparisonTable = createMethodComparisonTable(methodComparison, periods)
    % 원본 방법과 신뢰도 검증 방법의 비교 결과를 테이블로 정리하는 함수
    ...
    for i = 1:length(periodFields)
        ...
        if result.correlationImprovement > 0
            newRow.ImprovementStatus = {'향상'};
        elseif result.correlationImprovement < 0
            newRow.ImprovementStatus = {'감소'};
        else
            newRow.ImprovementStatus = {'동일'};
        end
