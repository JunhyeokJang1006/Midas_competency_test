%% 각 Period별 문항과 역량검사 종합점수 간 상관 매트릭스 생성 (Thorndike Case II 보정)
%
% 목적: 각 시점별로 수집된 문항들과 역량검사 종합점수 간의
%       전체 상관 매트릭스를 생성하고 분석 (Thorndike Case II 공식으로 보정)
%
% Thorndike Case II 보정: 개별 문항이 총점에 포함되어 발생하는
%                        부분-전체 상관 편향(part-whole correlation bias)을 보정
%
% 보정 공식: r_corrected = (r_observed - sqrt(1/k)) / sqrt(1 - 1/k)
%           여기서 k는 전체 문항 수, r_observed는 관찰된 상관계수
%
% 작성일: 2025년
...
fprintf('================================================================\n');
fprintf('Period별 문항-종합점수 상관 매트릭스 생성 (Thorndike Case II 보정)\n');
fprintf('================================================================\n\n');

% Thorndike Case II 보정 함수 테스트 실행
testThorndikeCaseII();
...
outputFileName = sprintf('D:\\project\\HR데이터\\결과\\성과종합점수&역검\\correlation_matrices_by_period_Thorndike_%s.xlsx', dateStr);
...
% MAT 파일로도 저장
matFileName = sprintf('D:\\project\\correlation_matrices_workspace_Thorndike_%s.mat', dateStr);
if ~isempty(fieldnames(correlationMatrices))
...
try
    ...
    % 상위항목 분석 수행
    upperCategoryResults = analyzeUpperCategoryPerformance(upperCategoryData, performanceResults, competencyTestData, periods, integratedPerformanceData);
...
function [correlationMatrix, pValues, cleanData, variableNames] = performCorrelationAnalysis(matchedData, questionCols)
    ...
    try
        % 원래 상관계수 계산
        correlationMatrix_original = corrcoef(cleanData, 'Rows', 'pairwise');

        % Thorndike Case II 보정 적용
        correlationMatrix = applyThorndikeCaseII(correlationMatrix_original, size(cleanData, 2));

        % p-value 계산 (보정된 상관계수 기준)
        n = size(cleanData, 1);
        ...
        % 보정 전후 비교 출력
        fprintf('  ▶ Thorndike Case II 보정 적용 완료\n');
        if size(correlationMatrix, 2) >= 2
            lastColIdx = size(correlationMatrix, 2);
            questionCorrs_original = correlationMatrix_original(1:end-1, lastColIdx);
            questionCorrs_corrected = correlationMatrix(1:end-1, lastColIdx);

            % 보정 효과가 큰 상위 3개 문항 출력
            correction_effect = abs(questionCorrs_corrected - questionCorrs_original);
            [~, effectIdx] = sort(correction_effect, 'descend');

            fprintf('  ▶ 보정 효과가 큰 상위 3개 문항:\n');
            for i = 1:min(3, length(effectIdx))
                idx = effectIdx(i);
                if idx <= length(questionCols)
                    qName = questionCols{idx};
                    orig_corr = questionCorrs_original(idx);
                    corr_corr = questionCorrs_corrected(idx);
                    effect = correction_effect(idx);

                    fprintf('    %s: %.3f → %.3f (변화: %.3f)\n', ...
                        qName, orig_corr, corr_corr, corr_corr - orig_corr);
                end
            end
        end
...
%% Thorndike Case II 보정 함수
function correctedMatrix = applyThorndikeCaseII(originalMatrix, numItems)
    % Thorndike Case II 공식: r_corrected = (r_observed - sqrt(1/k)) / sqrt(1 - 1/k)
    % 여기서 k는 전체 문항 수, r_observed는 관찰된 상관계수
    %
    % 입력:
    %   originalMatrix - 원래 상관계수 매트릭스
    %   numItems - 전체 문항 수
    %
    % 출력:
    %   correctedMatrix - Thorndike Case II로 보정된 상관계수 매트릭스

    if numItems <= 1
        correctedMatrix = originalMatrix;
        return;
    end

    % 보정 계수 계산
    k = numItems;
    correction_term = sqrt(1/k);
    denominator = sqrt(1 - 1/k);

    % 보정 적용 (대각선 요소는 보정하지 않음)
    correctedMatrix = originalMatrix;

    % 대각선이 아닌 요소들에 대해서만 보정 적용
    for i = 1:size(originalMatrix, 1)
        for j = 1:size(originalMatrix, 2)
            if i ~= j
                r_observed = originalMatrix(i, j);

                % 보정 공식 적용
                r_corrected = (r_observed - correction_term) / denominator;

                % 상관계수 범위 [-1, 1]로 제한
                r_corrected = max(-1, min(1, r_corrected));

                correctedMatrix(i, j) = r_corrected;
            end
        end
    end

    % 대각선 요소는 1로 유지
    correctedMatrix(logical(eye(size(correctedMatrix)))) = 1;
end

%% Thorndike Case II 보정 테스트 함수
function testThorndikeCaseII()
    fprintf('\n=== Thorndike Case II 보정 테스트 ===\n');

    % 테스트 데이터 생성 (5개 문항 + 1개 총점)
    ...
    % 총점 계산 (문항들의 평균)
    total_score = mean(items, 2);

    % 전체 데이터
    test_data = [items, total_score];

    % 원래 상관계수 계산
    original_corr = corrcoef(test_data);

    % Thorndike Case II 보정 적용
    corrected_corr = applyThorndikeCaseII(original_corr, k+1);

    % 결과 비교 출력
    fprintf('문항 수: %d, 샘플 수: %d\n', k, n);
    fprintf('보정 계수: sqrt(1/k) = %.4f\n', sqrt(1/(k+1)));
    fprintf('분모: sqrt(1-1/k) = %.4f\n', sqrt(1-1/(k+1)));

    fprintf('\n문항-총점 간 상관계수 비교:\n');
    for i = 1:k
        orig_r = original_corr(i, k+1);
        corr_r = corrected_corr(i, k+1);
        diff = corr_r - orig_r;

        fprintf('문항%d: %.4f → %.4f (변화: %+.4f)\n', ...
            i, orig_r, corr_r, diff);
    end

    % 평균 보정 효과
    avg_original = mean(abs(original_corr(1:k, k+1)));
    avg_corrected = mean(abs(corrected_corr(1:k, k+1)));

    fprintf('\n평균 절대 상관계수:\n');
    fprintf('원본: %.4f\n', avg_original);
    fprintf('보정: %.4f\n', avg_corrected);
    fprintf('평균 감소: %.4f\n', avg_original - avg_corrected);

    fprintf('\n✓ Thorndike Case II 보정 테스트 완료\n');
    fprintf('=================================\n\n');
end
...
function displayTopCorrelations(correlationMatrix, pValues, questionCols)
    ...
    fprintf('  상위 5개 문항의 종합점수와의 상관 (Thorndike Case II 보정됨):\n');
...
function displayPerformanceQuestionCorrelations(correlationMatrix, pValues, questionCols, performanceQuestions)
    ...
    fprintf('  성과 관련 문항의 역량검사 종합점수와의 상관 (Thorndike Case II 보정됨):\n');
