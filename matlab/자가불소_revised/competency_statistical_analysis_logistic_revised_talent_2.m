...
% 파일 경로 설정
config = struct();
config.hr_file = 'D:\project\HR데이터\데이터\역량검사 요청 정보\최근 3년 입사자_인적정보_revised_2.xlsx';
...
for i = 1:n_types
    ...
    % 그리드 그리기 (고정 눈금: 50, 60, 70, 80, 90)
    fixed_grid_values = [50, 60, 70, 80, 90];

    for j = 1:length(fixed_grid_values)
        grid_value = fixed_grid_values(j);

        % min_val~max_val 범위 내에서 정규화된 위치 계산
